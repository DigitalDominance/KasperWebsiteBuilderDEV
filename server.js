...
- Use a gradient of "${colorPalette}" plus the a "${themeSelection}" theme for the color scheming of the background and give an opposite contrast for the components. keep a consistent theming across the site, gradient and nice looks. 
...
- Separate sections in this order:
  1) Modern Looking glass Nav (non-sticky) with a 256x256 transparent token logo => "NAV_IMAGE_PLACEHOLDER" on the left side and on the right side some placeholder nav links that don't work. (Also repeated in footer as "FOOTER_IMAGE_PLACEHOLDER", same image). 
  2) Modern Big glass hero with a blurred bg image with "HERO_BG_PLACEHOLDER" (1024x1024). Must show coin name "${coinName}" and reference "${projectDesc}". Space them nicely though.
  3) A heading and subheading component and then under it a Vertical roadmap (5 glass steps). Fancy. Make sure their width is fitting to the screen size.
  ...
  5) A heading and subheading component and then under it Exchange/analytics with 6 glass placeholders (laid out nicely). Under the heading. 2 rows, 3 columns on computer that take up wide enough not so skinny it only takes up one part we need the whole section of the screen and, vertical layout for mobile. Under the heading.
  6) A heading and subheading component and then under it 2 glass-card about section. Beatiful looks
  7) glass Footer section at the bottom not sticky. Uses FOOTER_IMAGE_PLACEHOLDER on the left and on the right it uses placeholder social links that don't work.
